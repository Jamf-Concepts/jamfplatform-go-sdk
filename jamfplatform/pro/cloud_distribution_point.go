...
//
// Parameters:
//   - id: The identifier of the inventory file to be marked as failed. The type and ID
//     will make a unique identifier for the file.
//   - fileName: Name of the file to mark failure for.
//   - uploadType: Type of file to mark failure for. Possible values are PACKAGE, EBOOK,
//     MOBILE_DEVICE_APP.
...
//     separate with commas. Allows sort for id, fileName, inventoryId and type
//     etc.
//   - filter: Filters results. Use RSQL format for query. Allows for many fields,
//     including fileName and type Can be combined with paging and sorting. Fields
//     allowed in the query: fileName, inventoryId and type Default filter is an
//     empty query and returns all results from the requested page.
...
//
// Parameters:
//   - fileName: Name of the file to check the availability of. If available, the inventory
//     and status will be updated in Jamf Pro. If no file is specified, it will
//     force an immediate inventory refresh at a rate-limit of once every 15
//     seconds.
