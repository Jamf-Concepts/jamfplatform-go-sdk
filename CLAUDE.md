...
*Still pending, unlanded.* The `ConnectionConfigLeftRequest`/`ConnectionConfigRightRequest` renames, and `ConnectionConfigPatchLeftRequest`/`ConnectionConfigPatchRightRequest` bundled as `GatewayIpSecPatchRequest`. That second one is not cosmetic — see the PATCH note below, where its absence makes a server capability unreachable from Go.
...
- **The server accepts a partial `ipsec` merge-patch and the SDK cannot send one.** `{"ipsec":{"esp":{…}}}` returns 204 and deep-merges — `keyExchange`, `ike`, `left`, `right` are all preserved (curl-verified). But `GatewayPatchRequest.ipsec` reuses the POST-shaped `GatewayIpSecRequest`, whose `KeyExchange`/`Ike`/`Esp`/`Left`/`Right` are non-pointer because the spec marks them required, so a partial Go value marshals `keyExchange: ""` and empty sub-objects and earns the malformed-body 400. **Callers must resend the whole `ipsec` block**; omitting `left.secret` on the resend preserves the existing PSK. The pending `GatewayIpSecPatchRequest` is what fixes this properly. The acceptance test pins the 400 deliberately, so it fails the day that shape lands and the workaround should be removed — do not "fix" that assertion by deleting it.
