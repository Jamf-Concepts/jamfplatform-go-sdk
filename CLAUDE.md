...
The `testing/` source specs are gitignored (private). The generator falls back to the committed `api/*.json` published spec for any family whose `testing/` source is absent (`resolveSpecPath` in `emit.go`) — this is how CI regenerates Go from committed specs without the private sources, and why the App Installer specs keep working even though nobody re-sources them.

### Sourcing `testing/` from a GitOps archive
...
- The rule auto-handles the one naming quirk — `compliance-benchmarks`, whose privilege-bearing spec is `compliance-benchmarks-beta-beta` (plain `-beta` has none) — with no special-casing.

Two deliberate exceptions:

- **App Installers** (Titles / Deployments / Global Settings) are NOT in the archive (the `app-installers` paths are absent from jpapi too). They stay manually sourced in `testing/`; the `api/` fallback covers regeneration. The script never touches them.
- **jpapi version** tracks whatever the upstream repo has published, which lags the shipping Jamf Pro version by a minor (build v888 = 11.28.0; the hand-sourced spec was 11.29.0). Adopting an archive pins jpapi to that build. After `make generate`, confirm `JamfProAPIVersion` in `jamfplatform/version.go` is acceptable before committing — a stale build silently rolls back Pro (e.g. drops visionOS inventory types).
