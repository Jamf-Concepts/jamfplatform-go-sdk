...
- **Undocumented constraints, all enforced on create and PATCH alike:** `ipsec.left.subnets` must be a private range (`10.0.0.0/8` /8–/30, `172.16.0.0/12` /12–/30, `192.168.0.0/16` /16–/30) — the spec's own `0.0.0.0/0` examples are rejected — while `right.subnets` accepts public ranges; `contact` is required though `GatewayCreateRequest.required` omits it; `dedicatedIps.enabled: true` is mutually exclusive with `ipsec`; and a gateway must be one or the other.
- **`ipsec: null` on PATCH returns `400 IPSEC_REMOVAL_NOT_SUPPORTED`** — already live in production, though it only appears in the unlanded preview spec. `IPSEC_SECRET_CLEAR_NOT_SUPPORTED` fires for `left.secret: null` and does carry `field: "ipsec.left.secret"`.
