...
- `POST /ztna/apps`, `POST /ztna/grouped-gateways` **and `POST /ztna/gateways`** all answer **201 with the full resource object**, where the spec declares the shared `CreateResponse` (`{id, href}`) — of which the server sends only `id`, never `href`. All three carry a `responseType` override so callers get the created object rather than a reference to it. Gateways was the last unprobed one, on the grounds that creating a gateway provisions real egress infrastructure; it was probed on 2026-08-20 by creating one with `enabled: false` on the wisconsam sandbox and deleting it, and it returns the whole `Gateway` with no `Location` header. That also retires the "`ApplyZtnaGatewayV1`'s create branch is unverified" caveat below — the ID it returns is now an observed payload.
